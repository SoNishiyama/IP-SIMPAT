...
2. the potential paternal individuals have been identified (i.e., the closed mating system) 
...
### Overview
Directory `IP-SIMPAT-main/` containts all the MATLAB implementation for the proposed optimization method. Directory `util/` contains several scripts 
that would be used for input preparation and genotype simulation. Directory `test-data/` contains test data that is adoption of very valuable apple gnotype data by [Muranty et al. 2020](https://doi.org/10.1186/s12870-019-2171-6) 
...
```
`318` is an individual ID representing target maternal parent, 'Fuji', used in Muranty et al (2020). This needs to be changed according to the representation of your data.
...
w, adjacency weight control       A test weight that controls negative power on closely located marker pairs. q_kl = (|B_k-B_l |/D_k )^(-1/w). Large w reduces the negative power and thus physically close pairs can be more likely selected. My test yielded that w = 4, which add 10 times weight for a 3 kb distance pair in 30 Mb chromosome than a linkage equibrium pair, can be a good option to produce reasonable optimization, and the smaller produces safer solution. 
constthre                         constraint threshold, how many distinct loci (homoz ref vs homoz alt, plus info from heterogyzous loci depending on h) is needed for discriminating a pair of individual. Setting "1" worked in every optimization in our test. The larger produces safer solution, while more markers are required.
...
To see the effectiveness of the optimized marker set, it might be interesting to perform simulated crossing between the target maternal parent and each of potential paternal parent, and validate if the set successfully produce the parentage inference.
...
```
The above commands would produce simulated genotype of five individuals for each cross between Fuji (318) and each of 29 candidate paternal individuals.
